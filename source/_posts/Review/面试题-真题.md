...
异常采集 4w: WHO did WHAT and get WHICH exception in WHICH environment?
...
异常捕获方法
try catch 判断一段代码中是否存在异常，只能捕获到同步的运行时错误，对语法和异步错误却无能为力。
window.onerror全局监听异常，但不能捕获promise的异常错误信息。script error跨域时 script 加上属性crossorigin并且服务器添加
Access-Control-Allow-Origin。
window.addEventListener('unhandledrejection', event => ···) 可捕获到未处理的Promise错误。
sourceMap  在webpack中开启sourceMap功能追踪错误，node 中间层处理分析。
aixos中使用interceptor进行拦截
vue 使用 Vue.config.errorHandler 捕获全局异常
react 框架 引入了Error Boundary，componentDidCatch 中 logErrorToMyService 将异常信息上报给服务器


错误上报
ajax传递给接口
动态创建 img 标签的形式 new Image().src=reportUrl?logs=error 的方式进行上报
